...
exports.getBuffer = getBuffer = async (url) => {
	...
	const anu = fs.readFileSync('./src/error-404-not-found.jpg')
...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./tmp/@aine.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
        if (err) return console.error(err)
        return `./tmp/@aine.exif`
    })
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './tmp/@aine.exif', media, '-o', out])
        .on('exit', () => {
